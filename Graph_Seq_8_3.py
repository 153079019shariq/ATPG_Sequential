	...
	for i in  G.edges:
		if(G.nodes[i[0]]['type']=='FF'):
			if(G.edges[i]['fault']=='sa0'):
				...
				sa0_insert =0
			if(G.edges[i]['fault']=='sa1'):
				sa1_insert =1
			else:
				sa1_insert =0
			GU.remove_edge(i[0], i[1])
			if(sa0_insert ==1):
				GU.add_edge(i[0]+"_op", i[1],value_non_fault='x',value_faulty='x', fault='sa0',cc0=1,cc1=1,co=0)
			elif(sa1_insert==1):
				GU.add_edge(i[0]+"_op", i[1],value_non_fault='x',value_faulty='x', fault='sa1',cc0=1,cc1=1,co=0)
			else:
...
def Connect_FF_op_FF_ip(Graph,No_of_Unroll):
	...
	for i in range(len(list_ip_FF)):
			...
			print "list_inedge",list_inedge
			sa0_sa1_FF_ip	=Graph.edges[list_inedge]['fault']
			
			print "sa0_sa1",sa0_sa1_FF_ip
			...
			sa0_sa1_FF_op	=Graph.edges[list_outedge]['fault']
			print "sa0_sa1",sa0_sa1_FF_op
			sa0_sa1_FF=sa0_sa1_FF_op +sa0_sa1_FF_ip
			print "sa0_sa1_FF",sa0_sa1_FF_op +sa0_sa1_FF_ip
			Graph.remove_edge(list_outedge[0], list_outedge[1])
			Graph.remove_node(list_outedge[0])
			
		
			Graph.add_edge(list_inedge[0], list_outedge[1],value_non_fault='x',value_faulty='x', fault=sa0_sa1_FF_ip)
