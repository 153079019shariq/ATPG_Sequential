...
def controlabilty(G,node):
	list_predecessorCC0 =[]
	list_predecessorCC1 =[]
	
	if(G.nodes[node]['type']=='gate' or G.nodes[node]['type']=='fanout' ):
...
def overall_Controllability(G,list1):
	for node in list1:
		controlabilty(G,node)
	for node in list(reversed(list1)):
		observabiltiy(G,node)
	print_Graph_edges(G)
